...
export const getRoutes = async (saccoId: string): Promise<Route[]> => {
  const response = await api.get<Route[]>(`/saccos/${saccoId}/routes`);
  return response.data;
};

export const addRoute = async (saccoId: string, routeData: NewRouteData): Promise<Route> => {
  const response = await api.post<Route>(`/saccos/${saccoId}/routes`, routeData);
  return response.data;
};

// Vehicle Management
export const getVehicles = async (saccoId: string): Promise<Vehicle[]> => {
    const response = await api.get<Vehicle[]>(`/saccos/${saccoId}/vehicles`);
    return response.data;
};

export const addVehicle = async (saccoId: string, vehicleData: NewVehicleData): Promise<Vehicle> => {
    const response = await api.post<Vehicle>(`/saccos/${saccoId}/vehicles`, vehicleData);
    return response.data;
};

// Driver Management
export const getDrivers = async (saccoId: string): Promise<Driver[]> => {
    const response = await api.get<Driver[]>(`/saccos/${saccoId}/drivers`);
    return response.data;
}
